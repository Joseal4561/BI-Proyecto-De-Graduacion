...
const DatosEducativos = () => {
  ...
  const fetchData = async () => {
    try {
      ...
      setDatos([]); // Set empty array on error
    } finally {
      setLoading(false);
    }
  };
  ...
  const handleFileUpload = async () => {
    ...
    setIsProcessing(true);
    try {
      // Filter out rows without school ID
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      if (editingData) {
        ...
        await api.post('/datos-educativos', formData);
        ...
        <Modal.Body>
          <Alert variant="info">
            ...
            <ul className="mb-0 mt-2">
              <li><strong>escuelaId</strong> - ID de la escuela (número)</li>
